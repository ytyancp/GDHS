...
class GDHS_GL(TransformerMixin):
    ...
    def fit_transform(self, _X, _y, shuffle: bool = False):
        ...
        record_weight = defaultdict(float)
        ...
        for class_name, class_quantity in dsc_maj_cls:
            X, y = self._undersample(X, y, ori_X, class_name, record_weight, dsc_maj_cls)
    ...
    def _undersample(self, X, y, ori_X, this_class, record_weight, dsc_maj_cls):
        indices_in_class = [i for i, value in enumerate(y) if value == this_class]
        remove_num = len(indices_in_class) - self.goal_quantity
        remove_indices = []
        ...
        all_maj_weight_avg = 0.0
        for value in record_weight.values():
            if value != 0:
                all_maj_weight_avg = all_maj_weight_avg + value

        all_maj_weight_avg = all_maj_weight_avg/len(indices_all_maj_class)
        ...
        sorted_record_maj_weight = dict(sorted(record_maj_weight.items(), key=lambda item: item[1],reverse=True))
        for key, value in sorted_record_maj_weight.items():
            if value > all_maj_weight_avg:
                remove_indices.append(key)
            else:
                continue

        if len(remove_indices) > remove_num:
            slice_num = len(remove_indices) - remove_num
            remove_indices = remove_indices[:-slice_num]
        if len(remove_indices) < remove_num:
            ...
            for i, sample_id in enumerate(indices_in_class):
                for num in range(n_neigh):
                    if y[neighbor_indices[i][num]] != this_class:
                        count_diff[sample_id] = count_diff[sample_id] + 1
            count_diff = dict(sorted(count_diff.items(), key=lambda item: item[1], reverse=True))

            remove_maj_maj_indices = []
            for key, value in count_diff.items():
                if value >= 4 and key not in remove_indices:
                    remove_maj_maj_indices.append(key)
                else:
                    continue
            if len(remove_maj_maj_indices)>remove_num-len(remove_indices):
                slice_num = len(remove_indices)+len(remove_maj_maj_indices) - remove_num
                remove_indices = remove_indices[:-slice_num]
